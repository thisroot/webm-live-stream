...
import {writeToDisk, merge} from './record/index'
...
module.exports = exports = function (app, socketCallback) {
    ...
    function onConnection(socket) {
        ...
        // streamrec
        var NodeJsRecordingHandler = require('./streamrec/Nodejs-Recording-Handler.js');
        NodeJsRecordingHandler(socket);

        if (params.enableScalableBroadcast) {
            if (!ScalableBroadcast) {
                ScalableBroadcast = require('./broadcasting/scalable-broadcast.js');
            }
            console.log("max users", params.maxRelayLimitPerUser);
        ...
        socket.on('message', ctx => {
            ...
            writeToDisk(ctx.audio.dataURL, fileName + '.wav');

            if (ctx.video) {
                writeToDisk(ctx.video.dataURL, fileName + '.webm');
